  ...
  const steps: Record<Step, React.ReactNode> = {
    welcome: (
      <div className="text-center animate-fade-in">
        ...
        <p className="mt-4 text-gray-500 max-w-md mx-auto leading-relaxed">
          Isso aqui é temporário. Nosso objetivo é que você não precise mais de nós.
          Vamos configurar seu espaço juntos.
        </p>
        <button
          onClick={() => setStep('context')}
          className="mt-8 inline-flex items-center gap-2 px-6 py-3 bg-gray-900 text-white rounded-xl font-medium hover:bg-gray-800 transition-colors"
        >
    ...
    context: (
      <div className="animate-fade-in">
        ...
        <div className="mt-8 flex justify-between">
          <button onClick={() => setStep('welcome')} className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700">
            <ArrowLeft size={16} /> Voltar
          </button>
          <button
            onClick={() => setStep('cycle')}
            disabled={!context}
            className="inline-flex items-center gap-2 px-6 py-2.5 bg-gray-900 text-white rounded-lg text-sm font-medium hover:bg-gray-800 transition-colors disabled:opacity-30"
          >
          ...
          {CYCLE_OPTIONS.map((opt) => (
            <button
              ...
              className={`p-4 rounded-xl border text-center font-medium transition-all ${
                cycleMonths === opt.months
                  ? 'border-gray-900 bg-gray-50 text-gray-900'
                  : 'border-gray-200 text-gray-600 hover:border-gray-300'
              }`}
            >
              <span className="text-2xl font-bold block">{opt.months}</span>
              <span className="text-xs text-gray-500">meses</span>
            </button>
          ))}
        </div>
        <div className="mt-8 flex justify-between">
          <button onClick={() => setStep('context')} className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700">
            <ArrowLeft size={16} /> Voltar
          </button>
          <button
            onClick={() => setStep('first_goal')}
    ...
    first_goal: (
      <div className="animate-fade-in">
        ...
        <p className="mt-2 text-gray-500">Metas são o eixo central da plataforma. O que você quer conquistar?</p>
        <div className="mt-6 space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">O que você quer alcançar?</label>
            <input
              type="text"
              value={goalTitle}
              onChange={(e) => setGoalTitle(e.target.value)}
              placeholder="Ex: Juntar R$ 5.000 para emergência"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Tipo da meta</label>
        ...
        <div className="mt-8 flex justify-between">
          <button onClick={() => setStep('cycle')} className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700">
            <ArrowLeft size={16} /> Voltar
          </button>
          <button
            onClick={() => setStep('first_habit')}
            disabled={!goalTitle}
            className="inline-flex items-center gap-2 px-6 py-2.5 bg-gray-900 text-white rounded-lg text-sm font-medium hover:bg-gray-800 transition-colors disabled:opacity-30"
          >
            Continuar <ArrowRight size={16} />
      ...
      <div className="animate-fade-in">
        ...
        <div className="mt-6">
          ...
          <input
            ...
            placeholder="Ex: Ler 15 minutos, Meditar, Exercício"
            className="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent"
          />
        </div>
        <div className="mt-8 flex justify-between">
          <button onClick={() => setStep('first_goal')} className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700">
            <ArrowLeft size={16} /> Voltar
          </button>
          <button
    ...
    complete: (
      <div className="text-center animate-fade-in">
        ...
        <p className="mt-4 text-gray-500 max-w-md mx-auto">
          Seu espaço está configurado. Agora é com você. Lembre-se: o objetivo é que você não precise mais de nós.
        </p>
        <button
          onClick={handleComplete}
          disabled={loading}
          className="mt-8 inline-flex items-center gap-2 px-8 py-3 bg-gray-900 text-white rounded-xl font-medium hover:bg-gray-800 transition-colors disabled:opacity-50"
        >
